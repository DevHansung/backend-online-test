...
public class UserController {
	...
	public ResponseEntity<?> getCurrentUser(@PathVariable("name") String name) {
		...
		return ResponseEntity.ok().body(new StringApiRes(true, "등록된 회왼입니다."));
	}
	...
	public ResponseEntity<?> insertUser(@RequestBody User user) {
		...
		return ResponseEntity.ok().body(new StringApiRes(true, userDao.save(user).getName() + "회원의 회원등록이 완료되었습니다."));
	}
}